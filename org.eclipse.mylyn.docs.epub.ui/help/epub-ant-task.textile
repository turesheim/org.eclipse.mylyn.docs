...
<!ELEMENT epub (identifier | type | subject | reference | creator | meta | publisher | source | language | rights | contributor | format | cover | toc | item | date | title | fileset)*>
<!ATTLIST epub
          id ID #IMPLIED
...
* **automaticAdd** - optionally used to automatically include referenced items in the finished publication. When setting this to **true** there should be no need to explicitly add for instance image files.
